...
# Configuration - Add your Google Sheets URL here for auto-connection
AUTO_CONNECT_SHEET_URL = "https://docs.google.com/spreadsheets/d/12qpJ6hwD_dlLrTbj1uwund3m5uTLA8DCL8wdPuyJ4KM/edit?usp=sharing"  # Paste your Google Sheets URL here to enable auto-connection
# Example: AUTO_CONNECT_SHEET_URL = "https://docs.google.com/spreadsheets/d/1ABC123.../edit"
...
class GoogleSheetsConnector:
    ...
    def __init__(self, credentials_dict, spreadsheet_url, debug_mode=False):
    ...
    def extract_training_data(self):
        ...
        for week_num in range(1, 9):
            try:
                ...
                # Only show status messages when debug mode is explicitly enabled
                if exercises_this_week > 0 and self.debug_mode:
                    st.success(f"✅ Successfully read {exercises_this_week} exercises from {sheet_name}")
                        
            except Exception as e:
                # Only show errors when debug mode is explicitly enabled
                if "does not exist" not in str(e) and self.debug_mode:
...
def attempt_auto_connection():
    """Attempt to auto-connect to Google Sheets if configured"""
    if not AUTO_CONNECT_SHEET_URL:
        return False, "No auto-connect URL configured"
    ...
    try:
        connector = GoogleSheetsConnector(credentials, AUTO_CONNECT_SHEET_URL, debug_mode=False)
        if connector.connect():
            st.session_state['connected'] = True
            st.session_state['connector'] = connector
            st.session_state['auto_connected'] = True
            return True, "Successfully auto-connected"
        else:
            return False, "Connection failed"
    except Exception as e:
        return False, f"Auto-connection error: {str(e)}"
...
def main():
    ...
    # Try auto-connection on first load if not already connected
    if ('connected' not in st.session_state or not st.session_state['connected']) and AUTO_CONNECT_SHEET_URL:
        success, message = attempt_auto_connection()
        if success:
            st.session_state['auto_connect_status'] = "✅ Auto-connected to Google Sheets"
        else:
            st.session_state['auto_connect_status'] = f"⚠️ Auto-connect failed: {message}"
    ...
    with st.sidebar:
        ...
        if st.session_state.presentation_mode:
            ...
            # Show auto-connection status in presentation mode
            if 'auto_connect_status' in st.session_state:
                if "✅" in st.session_state['auto_connect_status']:
                    st.success(st.session_state['auto_connect_status'])
                else:
                    st.warning(st.session_state['auto_connect_status'])
        else:
            st.markdown("#### 🎛️ Advanced Filters")
            
            # Show connection debug info in technical mode
            if AUTO_CONNECT_SHEET_URL:
                st.info(f"🔗 Auto-connect URL configured: {AUTO_CONNECT_SHEET_URL[:50]}...")
                if 'auto_connect_status' in st.session_state:
                    st.write(f"Status: {st.session_state['auto_connect_status']}")
                st.write(f"Connected: {'✅' if st.session_state.get('connected', False) else '❌'}")
                st.write(f"Credentials available: {'✅' if get_google_credentials() else '❌'}")
            else:
                st.warning("🔗 No auto-connect URL configured. Set AUTO_CONNECT_SHEET_URL in code.")
        ...
        if not st.session_state.presentation_mode:
            ...
            # Update connector debug mode if it exists
            if 'connector' in st.session_state and st.session_state['connector']:
                st.session_state['connector'].debug_mode = debug_mode
            ...
            if st.session_state.get('connected', False):
                ...
                if AUTO_CONNECT_SHEET_URL:
                    st.caption("📊 Auto-connected to configured Google Sheets")
            else:
                st.info("🎯 **Demo Mode**: Showing sample training data to demonstrate capabilities.")
                if AUTO_CONNECT_SHEET_URL:
                    st.caption("⚠️ Auto-connection configured but not active - using demo data")
    
    # Main content area
    try:
        # Determine actual data source based on connection status
        if st.session_state.get('connected', False):
            actual_data_source = "Google Sheets (Live)"
        else:
            actual_data_source = "Sample Data (Demo)"
        
        # Load data based on actual connection status
        if actual_data_source == "Sample Data (Demo)":
            ...
            if 'connected' in st.session_state and st.session_state['connected']:
                ...
                if not data:
                    if not st.session_state.presentation_mode:
                        st.warning("No data found in Google Sheets - falling back to sample data")
                    data = load_sample_data()
            else:
                if not st.session_state.presentation_mode:
                    st.warning("Connection lost - using sample data")
                data = load_sample_data()
